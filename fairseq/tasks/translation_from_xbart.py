...
def load_langpair_dataset(
    ...
    if replace_token:
        replace_map = {src_dict.index("50256"): src_dict.eos()}
        offsets = [0]
        src_dataset = ReplaceDataset(src_dataset, replace_map, offsets)
    align_dataset = None
    if load_alignments:
...
@register_task("translation_from_xbart")
class TranslationFromXBARTTask(LegacyFairseqTask):
    """
    ...
    def add_args(parser):
        ...
        parser.add_argument('--add-mask-token', action='store_true',
                            help='add mask tokens, for finetuning from denoising task')
        parser.add_argument('--replace-token', action='store_true',
                            help='replace <A0> with </s> for sentence pairs')
        parser.add_argument('--prepend-bos', action='store_true',
                            help='prepend with a bos token')
        # fmt: on
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        self.datasets[split] = load_langpair_dataset(
            ...
            replace_token=self.args.replace_token,
    ...
    def build_dataset_for_inference(
        self, src_tokens, src_lengths, constraints=None
    ):
        return LanguagePairDataset(
            src_tokens,
            src_lengths,
